...
HPI_data['TX1yr'] = HPI_data['TX'].resample('A', how='mean')
    # You get 1 value per year for TX1yr and so will not see anything when plotted on daily scale

#HPI_data.dropna(how='all, 'inplace=True) #Removes rows where ALL columns are NaN (Default is 'any')
#HPI_data.fillna(method='ffill', inplace=True) #Takes data from past(ffill) or future(bfill)
print(HPI_data[['TX', 'TX1yr']].head())

HPI_data['TX'].plot(ax=ax1)
    # Can add label='' to say what the legend refers to it as
    # Plot things on the same axis using ax=

plt.legend(loc=4) # Put loc=4 inside legend function to relocate it to the bottom right 
plt.show()
